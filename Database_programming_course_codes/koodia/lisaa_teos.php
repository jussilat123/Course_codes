...
if (isset($_POST['luo_teos'])) {
	...
	$teos_id_kysely = "SELECT max(Id) FROM keskustietokanta.Teos";
    
    // jos tietokannassa ei ole vielä yhtäkään teosta, aloitetaan numerosta 1
	$tulos = pg_query($teos_id_kysely);
	$rivi = pg_fetch_array($tulos);
    if (!$tulos) {
        $teossid = 1;
    }
    else {
        $teossid = intval($rivi[0] + 1);
    }
	...
	if($teos_nimi == ""){
		...
		$kysely = "INSERT INTO keskustietokanta.Teos (id, nimi)
					VALUES ($teossid, '$teos_nimi')";
		$tulos = pg_query($kysely);
		if($teos_tekija != ""){
			$paivitys = "UPDATE keskustietokanta.Teos SET tekija = '$teos_tekija' WHERE Id = $teossid";
			$tulos = pg_query($paivitys);
		}
		if($teos_tyyppi != ""){
			$paivitys = "UPDATE keskustietokanta.Teos SET tyyppi = '$teos_tyyppi' WHERE Id = $teossid";
			$tulos = pg_query($paivitys);
		}
		if($teos_luokka != ""){
			$paivitys = "UPDATE keskustietokanta.Teos SET luokka = '$teos_luokka' WHERE Id = $teossid";
			$tulos = pg_query($paivitys);
		}
		if($teos_isbn != ""){
			$paivitys = "UPDATE keskustietokanta.Teos SET isbn = '$teos_isbn' WHERE Id = $teossid";
			$tulos = pg_query($paivitys);
		}
		echo "Teos lisätty! ID on: $teossid";
	}
...
if(isset($_POST['luo_nide'])){
	...
	//Tarkistetaan onko divarin id annettu
	$nide_divari_ID_query = $_POST['Nide_divari_ID'];
	if ($nide_divari_ID_query != ""){
		$nide_divari_ID = intval($nide_divari_ID_query);
		
		//Tarkistetaan onko kyseistä divaria olemassa keskustietokannassa.
		$divari_kysely = "SELECT Divari_ID FROM keskustietokanta.Divari WHERE Divari_ID = $nide_divari_ID";
		$divari_kysely_tulos = pg_query($divari_kysely);
		$divari_kysely_rivi = pg_fetch_array($divari_kysely_tulos);
		if(!$divari_kysely_rivi){
			echo nl2br ("Divaria $nide_divari_ID ei ole tietokannassa! \n");
			$tulos_ok = false;
		}
	} else {
		echo nl2br ("Divarin ID on annettava! \n");
		$tulos_ok = false;
	}
	...
	if($tulos_ok){
		$kysely = "INSERT INTO keskustietokanta.Nide (id, teos_id, paino, hinta, tila, divari_id)
					VALUES ($nide_id, $nide_teos_id, $nide_paino, $nide_hinta, '$nide_tila', $nide_divari_ID)";
		$tulos = pg_query($kysely);
...
<html>
 ...
 <body>
    ...
    <form action="lisaa_teos.php" method="post">

    <h2>Divarien ylläpitäjän lomake</h2>
	...
	<table border="0" cellspacing="0" cellpadding="3">
		...
		<tr>
    	    <td>Divari ID</td>
    	    <td><input type="text" name="Nide_divari_ID" value="" /></td>
	    </tr>
        <tr>
            <td></td>
	...
	// jos käyttäjä painoi 'Raportti 2', siirrytään raportin 2 näyttävälle sivulle
	if (isset($_POST['R2'])) {
    	header('Location: R2.php');
	}
	// Jos käyttäjä painoi 'Raportti 3', siirrytään raportin 3 näyttävälle sivulle
	if (isset($_POST['R3'])) {
...
pg_close($yhteys);
	...
	<br />
	<table border="0" cellspacing="0" cellpadding="3">
        <tr><td></td><td><input type="submit" name="R2" value="Raportti 2"/></td></tr>
        <tr><td></td><td><input type="submit" name="R3" value="Raportti 3"/></td></tr>
	</table>
